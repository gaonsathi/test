...
// ===================================================================
//  WEBSOCKETS — BOTH /curtain AND /live SHARE THE SAME RENDER PORT
//  -----------------------------------------------------------------
//  Render exposes the Web Service on one public HTTP/HTTPS port.
//  The previous version created a second HTTP server on LIVE_PORT
//  (8788). That port was never reachable through the Render URL, so
//  the browser connected to https://.../live on the MAIN service and
//  Render returned HTTP 400 during the WebSocket handshake.
//
//  We now keep both WebSocket endpoints on the same HTTP server and
//  route the upgrade by pathname:
//      wss://<same-render-host>/curtain
//      wss://<same-render-host>/live
//
//  This works with a single Render Web Service.
// ===================================================================

const server = http.createServer(app);

// Separate WebSocketServer instances, both attached to the SAME
// underlying HTTP server through manual upgrade routing.
const curtainWss = new WebSocket.Server({ noServer: true });
...
curtainWss.on('connection', (ws) => {
  ...
  // Keepalive ping so Render's proxy does not drop an idle socket.
...
// Live Voice uses the same public Render Web Service/port.
liveWss.on('connection', handleLiveConnection);

// IMPORTANT: route WebSocket upgrades BEFORE Express gets a chance to
// treat /live or /curtain as a normal HTTP request.
