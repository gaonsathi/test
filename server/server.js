...
// http.createServer (instead of app.listen) so the same port can serve
// both the normal REST API above AND the /live WebSocket endpoint.
const server = http.createServer(app);
const wss = new WebSocket.Server({ server, path: '/live' });
wss.on('connection', handleLiveConnection);

// ===================================================================
//  उद्घाटन ताला — REAL-TIME broadcast so every visitor's curtain opens
//  the instant VP sir taps "उद्घाटन करें", with no manual refresh.
//  -----------------------------------------------------------------
//  Every browser sitting on the locked curtain (intro/intro.js) opens
//  a plain WebSocket to /curtain and just listens — it never sends
//  anything meaningful. The moment /api/inaugurate/unlock succeeds
//  above, broadcastCurtainUnlock() pushes one small "unlocked"
//  message to every open /curtain connection, and each page runs its
//  ribbon-cutting animation live, in sync. A visitor who connects
//  AFTER the unlock already happened (e.g. joined mid-ceremony) gets
//  the current status immediately on connect instead, so they don't
//  miss it either. intro/intro.js also polls /api/inaugurate/status
//  every few seconds as a belt-and-braces fallback in case a
//  particular network blocks WebSockets.
// ===================================================================
...
curtainWss.on('connection', (ws) => {
  ...
  // Keep the curtain socket alive on Render (same 55-second idle timeout).
  // The curtain page just listens, never sends — so without a ping this
  // socket would be silently dropped and the real-time ribbon-cutting
  // broadcast would never reach late-arriving visitors.
...
server.listen(PORT, () => {
  ...
  console.log(`Live Voice WebSocket     → ws://localhost:${PORT}/live`);
