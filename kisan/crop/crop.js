...

{
  ...
  // ---------------------------------------------------------
  function lvWsUrl(){
    return AI_PROXY_BASE.replace(/^http/, 'ws') + '/live';
  }
  ...
  function lvConnect(){
    ...
    lvSocket.onclose = () => {
      ...
      if(lvSessionOpen){
        lvSetState('idle');
        lvSetStatus('कनेक्शन टूट गया। फिर से कोशिश करने के लिए बंद करके दोबारा खोलें।', 'Connection lost. Close and reopen to try again.');
      }
    };
  }
  ...
  // ---------------------------------------------------------
  let lvMicCtx = null, lvMicStream = null, lvMicSourceNode = null, lvMicProcessor = null, lvMicSending = false;
  ...
  async function lvStartMic(){
    ...
    // module file and works everywhere — plenty for chunking mic audio.
    ...
    lvMicProcessor = lvMicCtx.createScriptProcessor(512, 1, 1);
    const nativeRate = lvMicCtx.sampleRate;
    lvMicProcessor.onaudioprocess = (e) => {
      if(!lvMicSending) return;
      const pcm = floatTo16kPCM16(e.inputBuffer.getChannelData(0), nativeRate);
    ...
    silentGain.gain.value = 0; // must reach a destination for onaudioprocess to fire, but stay silent
  ...
  function lvStopMic(silent){
    ...
    if(lvMicProcessor){ try{ lvMicProcessor.disconnect(); }catch(e){} lvMicProcessor.onaudioprocess = null; lvMicProcessor = null; }
